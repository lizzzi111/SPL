...
library(plyr)
# this example of descriptive data will be done on the set with mathematics however exactly the same code can be used for
# portuguese or their mix
...
# simple vusualization for categorical data
...
library(plotly)
plot_ly(data, x = ~Walc, type = "histogram", marker = list(line = list(color = "white", width = 1))) %>% layout(title = "Alcohol Consumption during the Week")
print("Percent of students in each drinking level:")
table(data$Walc)/nrow(data)

# weekend
plot_ly(data, x = ~Dalc, type = "histogram", marker = list(line = list(color = "white", width = 1))) %>% layout(title = "Alcohol Consumption during the Weekend")
print("Percent of students in each drinking level:")
...
# Three way visualization To make it better to understand we will group G3 into 'didn't pass' 'good' 'perfect'
data$G3_grouped = factor(ifelse(data$G3 > 17.5, "excellent", ifelse(data$G3 > 15.5, "very_good", ifelse(data$G3 > 13.5, "good", 
    ifelse(data$G3 > 9.5, "sufficient", ifelse(data$G3 > 3.5, "week", "poor"))))))
library(ggplot2)
ggplot(data, aes(x = Dalc, y = Walc, col = G3_grouped, group = G3_grouped)) + stat_summary(fun.data = mean_cl_boot, geom = "errorbar", 
    width = 0.1, position = position_dodge(0.2)) + stat_summary(fun.data = mean_cl_boot, geom = "point", size = 3, position = position_dodge(0.2)) + 
    stat_summary(fun.data = mean_cl_boot, geom = "line", position = position_dodge(0.2))

# we can see that means within each alcohol consumption level is quite similar smart_anova function smart anova, x -
# numeric, y - categorical variable separating data into groups if distributions within groups are normal, and variances are
# homogen - apply anova if one of the criteria is not fullfilled - apply kruskal wallis test Idea: check whether group
# significantly differ from each other in pattern recognition, if they do - the feature is good
...
# therefore we can see that there is no significance diffirence or influence of the Alcohol on the grades we could also
# test, whether the level of alcohol drunk during the week and weekend are correlated which we could also assume
...
# we test whether two categorical variables are independent or not Input either x,y are the vectors of the same length, or y
# is not given and x contain a data frame of two categorical variables the default significance leve is 0.05
...
cdtest = function(x, y = NULL, alpha = 0.05) {
    ...
    if (any(l < 5)) {
        ...
        cat("The according test statistics, number of degrees of freedom and P-value are:\n", round(res$statistic, digits = 2), 
            round(res$parameter, digits = 2), round(res$p.value, digits = 2), sep = " ")
        return(c(res$statistic, res$parameter, res$p.value))
    }
}
cdtest(data$Walc, data$Dalc)

# as we cann see alcohol consumption during the week and during the weekend depend on each other
