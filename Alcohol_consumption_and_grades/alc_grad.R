...
graphics.off()
# Interactive visualizations (without custom graph) alcohol consumption
# week
library(plotly)
...
# Visualizations alcohol and grades 
# boxplots alcohol, grade within one term
...
# To make it better to understand we will group G3 into "didn't pass" "good" "perfect"
data$G3_grouped = factor(ifelse(data$G3>17.5, "excellent", 
                         ifelse(data$G3>15.5, "very_good",
                                ifelse(data$G3>13.5, "good",
                                       ifelse(data$G3>9.5, "sufficient",
                                              ifelse(data$G3>3.5, "week", "poor"))))))
library(ggplot2)
ggplot(data, aes(x = Dalc, y = Walc, col = G3_grouped, group = G3_grouped))+
...
# we could also test, whether the level of alcohol drunk during the week and weekend are correlated
# which we could also assume
