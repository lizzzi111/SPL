...
# 1) linearity 2) Strict exogenity 3) Full rank (check
# multicollinearity) 4) Spherical errors (homoscedasticity)


# 1) linearity
pairs(data[, c("G3", "G2", "G1")])
# relationships between variables look quite linear, however we can see
# some outliers
data_out = data[data$G2 > 5 & data$G3 > 5 & data$G1 > 5, c("G3", "G2", 
                                                           "G1")]
pairs(data_out)
...
# gets better but not normal however we need that the mean would be
# approximately zero
mean(fit$residuals)
...
# p-value > 0.05, we do not reject the zero hypothesis, the
# distribution of residuals is random, Runs test for randomness
lmtest::dwtest(fit)
...
hetero_test <- function(dep, indep) {
  ...
  ifelse(res_fit$p.value > 0.05, return("Assumption of homoscedasticity holds"), 
...
# that looks bad, assumption does not hold, however we can still use
# linear regression, but we have to keep in mind that standard errors
# are wrong, whereas, coefficients won't change.
