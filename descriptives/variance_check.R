...
variance_check = function(df, goal_var){
  ...
  # we divide into factors and non factors since they will be handled differently
  ...
  # therefore we would aim to look for constants. If some variable has only 
  # one unique value, it won't give us any additional predictive power.
  # It would only take computational power into account.
  # Especially in the cases where we have some similarity matrices.
  
  const_fac = check_unique(factors_df)
  
  # for numerics we simply check the variance
  # we took 0.09 as a threshold, 
  ...
  if(length(const_fac)==0 & length(n[n==TRUE])==0){
    ...
    cat("The variables ", names(n[n==TRUE]), "&", names(const_fac), " has only one unique value.\n")
