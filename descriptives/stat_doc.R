...
cdesc = function(df, goal_var, dropDuplicates = FALSE, cor_cutoff = 0.9){
  ...
  # zero variance predictors: we want to identify and probably drop predictors with small predictive power
  # load customized function
  source("./descriptives/variance_check.R")
  suspicious_variables = variance_check(df, goal_var)
  
  # duplicates
  # make sure we don't have duplicates in the data
  dup = duplicated(df)
  num_dup = dup[dup]
  ...
  # we change diagonal elements to false, since corelation of the variable to itself is always 1
  # and it won't give us any additional information
  diag(highCor) = FALSE
  pairs = apply(
    highCor, 1, 
  ...
  if(length(suspicious_variables)==0 & all(sapply(pairs, is.na))){
    ...
    output = list(nearZerovars = suspicious_variables, summary = summary, correlation = cor)
  } else if (length(suspicious_variables)==0 & !all(sapply(pairs, is.na))){
    output = list(summary = summary, correlation = cor, highlyCorPredictors = pairs)
  } else {
    output = list(nearZerovars = suspicious_variables, summary = summary, correlation = cor, highlyCorPredictors = pairs)
  }
  return(output)
}
